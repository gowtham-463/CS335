class Sym_tab :
		 def __init__(self):	
			...
			self.functionList = {'head' : self.sym_tab['head']}
		 ...
		 def add_Sc(self, name):
			currentScope = self.scope[len(self.scope) - 1]
			level = currentScope['__level__'] + 1
			typical = {
					'__scope__': name, 
					'__parentscope__': currentScope['__scope__'],
					'__returnType__': 'UNDEFINED',
					'__type__': 'FUNCTION',
					'__stringList__' : [],
					'__level__' : level
					}
			currentScope[name] = typical
			self.scope.append(currentScope[name])
			self.offset.append(0)
			self.functionList[name] = currentScope[name]


		 def get_name(self) :
			self.count = self.count + 1
			return '__fun'+str(self.count) + '__'

		 def getCurrentScope(self):

			return self.scope[len(self.scope)-1]['__scope__']
