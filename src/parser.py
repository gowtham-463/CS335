...
def p_compstmt_1(p):
	...
	p[0] = {}
	p[0]= p[1]
	p[0]['ptr'] = Node( 'Program',p[1]['ptr'],p[3]['ptr'])
...
def p_ARG_operations(p) :
	...
	p[0]['ptr'] = Node(p[2],p[1]['ptr'],p[3]['ptr'])
	types = ''
	if p[2] == '*' :
		p[2] = 'X'
	...
	if not 'type' in p[1] or  not 'type' in p[3]:
			print "VARIABLE HAS NOT BEEN INITIALIZED"
			exit(0)
...
def p_ARG_relational(p) :
	...
	p[0]['ptr'] = Node('=',Leaf('id',val),p[3]['ptr'])
	...
	if sym_tab.exists(p[1]):
		...
		#print sym_tab.scope[len(sym_tab.scope)-1]
...
s = '''b = 10; x = 30 + 20; y = x + b;'''

sys.stdout = open('AST.txt', 'w')
			
result = parser.parse(liness, lexer = lexer, debug = log)

sys.stdout = open('Symbol_table.txt', 'w')

#print sym_tab.scope[len(sym_tab.scope)-1]['__level__']
(sym_tab.scope[len(sym_tab.scope)-1].keys()).remove('b')
li =  sym_tab.scope[len(sym_tab.scope)-1].keys()

print "IDENTIFIERS IN THE PROGRAM--"
for elem in li :
	if not elem in ['__scope__','__returns__','__stringList__','__level__','__parentscope']:
		print elem

print '\n\n'

print "SCOPE OF THE PROGRAM"
print sym_tab.scope[len(sym_tab.scope)-1]['__scope__']
